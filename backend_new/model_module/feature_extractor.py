...
class ModelConfig:
    """模型配置类，内部配置不依赖外部"""
    def __init__(self):
        self.device = "cpu"  # 使用 "cuda" 或 "cpu"
        self.pretrain = True  # 是否加载预训练模型
        self.model_type = "resnet50"  # torchvision.models 里的模型名
        self.input_size = 224  # 输入图像的大小
        self.batchsize = 128
        self.normalize_mean = [0.485, 0.456, 0.406]  # 图像预处理的均值
        self.normalize_std = [0.229, 0.224, 0.225]   # 图像预处理的标准差


class feature_extractor(object):
    def __init__(self, config=None):
        """
        外部接口：初始化，使用内部配置或传入的配置
        :param config: 可选的配置对象，如果不提供则使用默认内部配置
        """
        if config is None:
            config = ModelConfig()
